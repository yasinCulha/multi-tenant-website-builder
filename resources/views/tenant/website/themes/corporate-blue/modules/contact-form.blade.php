<section id="iletisim" class="cb-contact">
    <div class="cb-container cb-contact__grid">
        <div>
            <span class="cb-eyebrow">Iletisim</span>
            <h2>{{ data_get($settings ?? [], 'contact.section_title', 'Bizimle Iletisime Gecin') }}</h2>
            <p>{{ data_get($settings ?? [], 'contact.section_subtitle', 'Projeniz icin kisa bir mesaj birakin, size donelim.') }}</p>
        </div>
        <form class="cb-form" method="post" action="#">
            ...
            <button class="cb-button" type="submit">{{ data_get($settings ?? [], 'contact.form_submit_button', 'Talebi Gonder') }}</button>
        </form>
    </div>
</section>
